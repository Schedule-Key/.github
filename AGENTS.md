...
Codex note: Codex sessions record a `cwd` but no git branch, so work done in Codex is not
issue-attributable and will report `UNMEASURED`. Log those hours by hand and say so.
